...
| 6 | Web | `FeeRow.svelte:61` ← `SigningHost.svelte:43` | The signing sheet's fee row, on a chain with exactly one fee coin. |open — web |
...
| 9 | iOS | `ConnectionPanelView.swift:58` ← `ExploreScreen.swift:577` | "Switch account" in the dApp connection sheet, chevron and all. Android draws the same row with no click either; web wires it. |**not a wiring slip** — no shell passes it, web included; see the question below |
...
| 11 | Desktop | `settings/components.rs:873`, `page.rs:7793` | Settings → Storage: every per-row 清除 and 清除全部缓存 are plain `div`s. (Same block: only the total is live, every per-row meta is fixture.) |open — needs live per-row data and a confirmation first; the per-row meta is still fixture |
...
| 15 | Web | `ActivityRow.svelte:23` ← `ContactsHome.svelte:216`, `ContactDetailPanel.svelte:86` | A contact's recent-activity rows press in under your finger and do nothing. |open — web |
...
| 17 | Desktop | `page.rs:10491` | The signing sheet's "Advanced — view raw data" chevron: the only route from the decoded summary to the raw payload before signing. Reads as not-yet-built, but it ships drawn in the live sheet. |open — this one is not wiring: desktop has no technical-details renderer, only the row |
| 18 | Web | `TxDetail.svelte:59`, `AssetDetailPanel.svelte:88`, `TokenDetail.svelte:79` | "View on explorer" drawn identically to the working `<a>` when there is no hash or no explorer URL. The source says it "draws the control inert"; nothing makes it look inert. |open — web |
| 19 | Web | `AssetRow.svelte:24` ← `SendForm.svelte:108` | Sweep form: each asset row is an enabled pressable button that does nothing (only its nested Max works), and `<button>` inside `<button>` is invalid HTML. |open — web |
