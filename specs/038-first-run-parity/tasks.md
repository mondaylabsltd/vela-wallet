...
- [x] T073 [US9] Reproduce #191 on the running web app (`pnpm dev`, parallel space, a history-derived row → Edit); record the exact failing step in `specs/038-first-run-parity/results.md`
- [x] T074 [US9] (nothing to fix on this branch — naming works; recorded in results.md) Fix per the reproduction: make "name this contact" the primary action on an `auto` row's detail in `app-web/vela-wallet/src/lib/contacts/live.ts` (+ `ui/`), saving via the core's `Save` (promotes to `Manual`); add a Vitest case in `app-web/vela-wallet/src/lib/contacts/live.test.ts` — SC-439

- [x] T074a [P] [US9] Add `og-image.svg` (1200×630: sailboat mark centred on `color.bg.base`, no text) to `app-web/vela-wallet/static/` and a `scripts/gen-og-image.mjs` (resvg-js or sharp, already in devDeps? else `@resvg/resvg-js`) that writes `static/og-image.png`; wire it into the `build` script — #meta
- [x] T074b [US9] (done WITHOUT a new key: `og:description` is `metaDescription` clipped at a word boundary to 110 chars — recorded deviation, avoids the 15-locale gate for a preview line) Add `metaSocialDescription` (≤ 110 chars) to all 15 locales in `rust/crates/vela-core/i18n/locales/*/…` where `metaDescription` lives, following `reference_i18n_corpus_gates` (six steps: corpus, paths.rs, vectors, wasm bytes + fingerprint, tests); regenerate — #meta
- [x] T074c [US9] `app-web/vela-wallet/src/lib/ui/SocialMeta.svelte` rendering `og:site_name`, `og:type=website`, `og:title`, `og:description`, `og:url`, `og:image` (+ `:width`/`:height`), `og:locale` (+ `:alternate` for the other 14), `twitter:card=summary_large_image`, `twitter:title`, `twitter:description`, `twitter:image`; used from `routes/[locale]/+page.svelte`'s `<svelte:head>` — SC-440
- [x] T074d [P] [US9] e2e in `app-web/vela-wallet/e2e/welcome-ssr.e2e.ts`: the prerendered HTML of `en` and `zh-CN` contains every tag above; `og:description` length ≤ 110; `og-image.png` responds 200 with `image/png` — SC-440
...
- [x] T074p [US12] (client done both shells: `aaguidDirectoryURL` in the service-endpoints object, core `directory_lookup_url_at` / `directory_entry_at`, wasm exports take the origin, desktop lookup now on the proxy chain; server task = biubiu-projects, not this repo) #E4: client — the AAGUID lookup base URL comes from the same settings object as the index (default = our node once it exists); server task filed in `biubiu-projects`
...
- [x] T074x [US12] (founder's ruling 2026-09-11: "best practice and the person's experience") batch fiat→token conversion rounds to six places, or the cent place when finer, capped by the token — core `conversion_decimals` + tests
- [x] T074y (same ruling) the signed-in frame and the onboarding frame cap at `--layout-frameMax` (1920) and centre past it — the four desktop roots, the intro, Welcome, FlowShell; overflow e2e extended to 3840
- [ ] T074z (same ruling; not this repo) the p256 index answers an unknown `walletRef` with 404 or an empty body, not 400 — biubiu-projects
