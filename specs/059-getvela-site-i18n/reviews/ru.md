...
### ru-1 — `home.signing.*` and `home.tradeoffs.items[*].body` — accuracy — **High** — fixed
...
`ru` kept the retired version: the old heading (`Подписывайте то, что видите.`) and single-paragraph
trade-offs. So this locale was still making, in Russian, exactly the claim the
...
### RU-2 — `home.hero.subtitle` — mistranslation — **High** — fixed

See [single-string.md](./single-string.md) RU-1. `Подписывайте passkey` reads as
"sign the passkey" — the mechanism stated backwards, in the one line under the
headline.
...
repair, not by a native speaker of Russian. The bar this project set is a reader
