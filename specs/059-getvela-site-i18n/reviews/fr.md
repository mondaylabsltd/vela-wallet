...
The sixteen docs are not translated into `fr` yet; they render English under a
notice, which is the designed fallback, not a finding.

## Findings

### fr-1 — `home.signing.*` and `home.tradeoffs.items[*].body` — accuracy — **High** — fixed
...
`fr` kept the retired version: the old heading (`Signez ce que vous voyez.`) and single-paragraph
trade-offs. So this locale was still making, in French, exactly the claim the
...
repair, not by a native speaker of French. The bar this project set is a reader
