# R7 review — `pt-BR` (Brazilian Portuguese)
...
The sixteen docs are not translated into `pt-BR` yet; they render English under a
notice, which is the designed fallback, not a finding.

## Findings

### pt-BR-1 — `home.signing.*` and `home.tradeoffs.items[*].body` — accuracy — **High** — fixed
...
`pt-BR` kept the retired version: the old heading (`Assine o que você vê.`) and single-paragraph
trade-offs. So this locale was still making, in Brazilian Portuguese, exactly the claim the
...
repair, not by a native speaker of Brazilian Portuguese. The bar this project set is a reader
