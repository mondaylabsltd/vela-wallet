...
The sixteen docs are not translated into `ko` yet; they render English under a
notice, which is the designed fallback, not a finding.

## Findings

### ko-1 — `home.signing.*` and `home.tradeoffs.items[*].body` — accuracy — **High** — fixed
...
`ko` kept the retired version: the old heading (`보이는 것에 서명하세요.`) and single-paragraph
trade-offs. So this locale was still making, in Korean, exactly the claim the
...
repair, not by a native speaker of Korean. The bar this project set is a reader
