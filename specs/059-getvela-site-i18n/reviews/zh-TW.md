# R7 review — `zh-TW` (Traditional Chinese)
...
The sixteen docs are not translated into `zh-TW` yet; they render English under a
notice, which is the designed fallback, not a finding.

## Findings

### zh-TW-1 — `home.signing.*` and `home.tradeoffs.items[*].body` — accuracy — **High** — fixed
...
`zh-TW` kept the retired version: the old heading (`所見即所簽。`) and single-paragraph
trade-offs. So this locale was still making, in Traditional Chinese, exactly the claim the
...
### zh-TW-2 — `home.seal.verify` — terminology — **Medium** — fixed

註冊表 is the Windows registry; an on-chain list of wallets is 鏈上登記. Same
finding as ZH-1, same fix, applied in the locale's own wording.
...
repair, not by a native speaker of Traditional Chinese. The bar this project set is a reader
