...
The sixteen docs are not translated into `it` yet; they render English under a
notice, which is the designed fallback, not a finding.

## Findings

### it-1 — `home.signing.*` and `home.tradeoffs.items[*].body` — accuracy — **High** — fixed
...
`it` kept the retired version: the old heading (`Firma quello che vedi.`) and single-paragraph
trade-offs. So this locale was still making, in Italian, exactly the claim the
...
repair, not by a native speaker of Italian. The bar this project set is a reader
