...
The sixteen docs are not translated into `ja` yet; they render English under a
notice, which is the designed fallback, not a finding.

## Findings

### ja-1 — `home.signing.*` and `home.tradeoffs.items[*].body` — accuracy — **High** — fixed
...
`ja` kept the retired version: the old heading (`見たとおりに署名する。`) and single-paragraph
trade-offs. So this locale was still making, in Japanese, exactly the claim the
...
repair, not by a native speaker of Japanese. The bar this project set is a reader
