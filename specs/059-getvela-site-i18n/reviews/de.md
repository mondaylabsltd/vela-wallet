...
The sixteen docs are not translated into `de` yet; they render English under a
notice, which is the designed fallback, not a finding.

## Findings

### de-1 — `home.signing.*` and `home.tradeoffs.items[*].body` — accuracy — **High** — fixed
...
`de` kept the retired version: the old heading (`Signiere, was du siehst.`) and single-paragraph
trade-offs. So this locale was still making, in German, exactly the claim the
...
repair, not by a native speaker of German. The bar this project set is a reader
