...
The sixteen docs are not translated into `id` yet; they render English under a
...
market's own product copy uses (formal `Anda`).
...
speaker of Indonesian has read it. That is the whole distinction between `drafted`
and `reviewed` here.
