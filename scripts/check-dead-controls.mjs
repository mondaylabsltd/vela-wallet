...
for (const [name, view] of views) {
  ...
  for (const [file, text] of source) {
    for (const call of callsOf(text, name)) {
      ...
      if (file === view.file && view.body.includes(call.args.slice(0, 40)))
        continue;
      anywhere++;
      if (!isDrawing(file) && !inPreview(text, call.at))
        live.push({ file, call });
    }
  }
  if (!live.length) {
  ...
  for (const action of view.actions) {
    ...
    const passed = live.some(
      ({ call }) =>
        ...
        (call.trailing && isLastAction(view, action)),
    );
    if (!passed) {
...
if (asJson) {
  console.log(
    JSON.stringify(
      { checkedViews, drawingOnly, allowed, findings },
      null,
      "\t",
    ),
  );
