//
//  ClearSignerDeviceTests.swift
//  VelaWalletUITests
//
//  Spec 071 on the phone, quickstart C4: a page asks for a signature, the
//  sheet starts at the stored "Sign with" — the Clear Signer — and the slide
//  opens the signer page in the in-app tab; closing the tab without signing
//  brings back the sheet, unsigned, with the Clear Signer's own sentence.
//
//  **Nothing here signs.** The page is shown and closed; no passkey ceremony
//  runs. Opt-in, because the signer page must be served:
//
//      cd app-web/clearsigning && python3 -m http.server 8141 &
//      TEST_RUNNER_VELA_CLEAR_SIGNER_PAGE=http://localhost:8141/ \
//        xcodebuild test … -only-testing:VelaWalletUITests/ClearSignerDeviceTests
//
...
final class ClearSignerDeviceTests: XCTestCase {
    ...
    private let signerPage = ProcessInfo.processInfo.environment["VELA_CLEAR_SIGNER_PAGE"]

    override func setUpWithError() throws {
        continueAfterFailure = false
        try XCTSkipIf(signerPage == nil, "set TEST_RUNNER_VELA_CLEAR_SIGNER_PAGE to a served signer page")
    ...
    func testClosingTheSignerPageLeavesTheRequestUnsigned() throws {
        ...
        app.launchArguments += ["-vela.signMethod", "clear_signer", "-vela.clearSignerUrl", signerPage ?? ""]
        app.launch()
        ...
        let signWith = app.descendants(matching: .any).matching(
            NSPredicate(format: "label CONTAINS %@ AND label CONTAINS %@", "签名方式", "清晰签名器")
        ).firstMatch
        XCTAssertTrue(signWith.waitForExistence(timeout: 30), "the sheet did not start at the Clear Signer")
        signWith.tap()
        XCTAssertTrue(app.staticTexts["在独立的页面上核对并签名——所见即所签。"].waitForExistence(timeout: 5),
                      "the Clear Signer's line is not under it")
        attach(XCUIScreen.main.screenshot(), named: "clear-signer-picker")
        signWith.tap()
        ...
        attach(XCUIScreen.main.screenshot(), named: "clear-signer-page")
        ...
        let closed = app.staticTexts["清晰签名器已关闭，没有签名。"]
        XCTAssertTrue(closed.waitForExistence(timeout: 20),
                      "closing the tab did not bring the sheet back, unsigned, with the Clear Signer's sentence")
        ...
        attach(XCUIScreen.main.screenshot(), named: "clear-signer-closed")
        app.terminate()
    }

    /// 设置 › 高级: "Sign with" and the Clear Signer page, beside the speed.
    ...
    func testSettingsShowsHowThisDeviceSigns() throws {
        ...
        XCTAssertTrue(app.staticTexts["清晰签名器"].exists, "the row does not say the stored method")
        ...
        app.staticTexts["清晰签名器页面"].firstMatch.tap()
