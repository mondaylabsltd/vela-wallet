...
四個應用程式都完整支援這四個欄位，而且改過的端點在下一次呼叫時就生效，不必等下一次啟動：
建立錢包、登入、替位址查名字——每一條路徑都是在真正用到端點的那一刻才去讀它。
在 iOS 上，預設索引連不上時，還可以在登入畫面更改密碼金鑰索引。

（在 2026 年 9 月之前，這件事有四項例外，
其中最嚴重的是 iOS 上那個顯示預留值、又什麼都不儲存的頁面。它們都已經修好了。）
...
兩條路都走得通：已發布的映像檔最省事，而 `docker compose up --build`
是從你能讀到的原始碼建置出同樣的東西。不用 Docker 的話，
可以用 `cargo run --release --bin vela-relay` 直接執行。
...
`P256_INDEX_DOMAIN_REGISTRY` 是最容易被漏掉的一項：少了它，
伺服器發出的挑戰會被合約拒絕，每一筆註冊都會失敗。它就在 `.env.example` 裡，
而且必須和已部署合約自己的 `DOMAIN_REGISTRY` 一致——
從註冊表 VERSION 12 起，挑戰所用的網域在部署時就固定下來，合約重新部署也不會變。
...
TLS 代理。`docker build -f p256-index-server/Dockerfile .` 從全新複製的儲存庫也能建置成功；
如果用 Compose，請先把 `.env.example` 複製成 `p256-index-server/.env`。
...
[網路與費用](/zh-TW/docs/networks-and-fees)說明了具體條件。
這項檢查已經涵蓋多把金鑰的錢包所需的那兩份合約，而且會特別標明它們——
一條鏈上沒有它們，單一金鑰的錢包照樣能跑。
