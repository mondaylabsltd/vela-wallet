...
密碼金鑰簽章由鏈上的 **RIP-7212** 預編譯合約驗證，沒有備用驗證器。啟用一條網路之前，應用程式會用一個真實
...
Vela 的應用程式從來不會組出更換擁有者的操作，所以 Vela 自己不會觸發這個問題。但它仍然重要：dApp 可以請求
你的錢包更換自己的擁有者（請看下方「缺口」），而任何人透過其他 Safe 工具移除一把被盜的金鑰時，也不能
指望它在同一批次裡就被切斷。
...
- **錢包呼叫自己的操作不會被攔截。** dApp 可以請求在你自己的 Safe 上執行 `enableModule`、
  `addOwnerWithThreshold`、`setFallbackHandler` 或 `setGuard`；其中任何一個，只要簽一次，就會把
  帳戶交出去。Vela 會解碼這些呼叫，但不會阻止。請拒絕任何目標是你自己位址的請求。
- **授權防護只擋「無上限」的金額**（2^200 以上；Permit2 則是 2^152）。金額很大但有上限的授權、簽章式授權，
  或 NFT 的 `setApprovalForAll`，只會附上警示，不會被擋下。
- **取得的描述檔沒有經過驗證。** 來自鏈資料伺服器的描述檔，只要和合約相符就會顯示為「已驗證」；
  它的可信程度取決於那台伺服器。
- **獨立簽署頁還沒有接到**任何應用程式。
- **網路檢查不會檢查 Safe 的密碼金鑰簽署器工廠合約**，而第二到第七把金鑰都需要它；在沒有它的情況下新增的
  網路上，只有第一把金鑰能簽署。
