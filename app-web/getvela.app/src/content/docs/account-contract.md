...
| [Safe 4337 Module v0.3.0](https://github.com/safe-global/safe-modules/tree/main/modules/4337) | Lets the EntryPoint operate the Safe; also its fallback handler | Safe |
| [SafeWebAuthnSharedSigner v0.2.1](https://github.com/safe-global/safe-modules/tree/main/modules/passkey) | Verifies the first key's P-256 signatures | Safe |
| [SafeWebAuthnSignerFactory v0.2.1](https://github.com/safe-global/safe-modules/tree/main/modules/passkey) and the signers it creates | One small signer contract per additional key | Safe |
...
account. A fork is a new contract with an old reputation.

**The audits apply to what you are actually using.** Safe's audit reports cover
these exact releases. A fork's audits cover the code before the fork; the
modification is the part nobody audited.
...
**The attack surface is one everybody else is also watching.** A bespoke account
contract is watched mostly by its author. This one is watched by everybody who
holds money in a Safe.
...
- **Gas.** Your signature is verified on-chain and the transaction runs through
  the EntryPoint. A simple send from a Vela wallet uses roughly 140,000–170,000
  gas once the wallet is deployed; a plain ETH transfer from an ordinary account
  uses 21,000. On top of the gas, the relay charges its fee — see
...
- **Not every chain qualifies.** Passkey signatures are verified with the
  **RIP-7212** precompile. Vela refuses to enable a network without it rather
  than fall back to a slower verifier.
