...
一台支持通行密钥的设备（带面容、指纹或 Windows Hello 的较新手机或电脑），或者一把 USB/NFC 安全
密钥。不需要邮箱、不需要注册账户、也不需要预先充值。创建时最多可以加七把钥匙，之后不能再加。见
...
- **App：**网页钱包、浏览器扩展和桌面版免费。iOS 和 Android App 将在商店以一次性买断的方式销售；
  你也可以免费从源码编译任何一个 App。
- **每笔交易：**从你的钱包付给提交它的中继一笔手续费。它包含 gas 和中继的利润，通常是这笔交易
  链上成本的好几倍，最低约 0.01 美元。确切金额显示在确认页上，并写在你签名的内容里。没有押金，
...
Vela 动不了你的资金，也读不到你的钥匙。但它能看到它的服务能看到的东西：登记新钱包时，索引能看到
你的公钥和钱包名称；中继能看到你的地址和你提交的操作；链数据服务能看到你的 App 查询了哪些代币和
合约。哪些内容会公开在链上，见[创建钱包](/zh/docs/create-wallet#what-is-public)。完整、权威的说明
以[隐私政策](/privacy)为准。
...
getvela.app 也能继续用，每一项服务都能替换。[自托管指南](/zh/docs/self-hosting#if-getvela-app-disappears)
列出了每条路及其限制。
