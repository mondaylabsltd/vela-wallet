...
四个 App 都完整支持这四个字段，而且改过的端点在下一次调用时就生效，不必等下一次启动：
创建钱包、登录、按地址查名字——每条路径都在真正用到端点的那一刻才去读它。
在 iOS 上，默认索引连不上时，还可以在登录界面修改通行密钥索引。

（在 2026 年 9 月之前，这件事有四条例外，
其中最严重的是 iOS 上那个显示占位数据、又什么都不保存的页面。它们都已经修好了。）
...
两条路都走得通：已发布的镜像最省事，而 `docker compose up --build`
是从你能读到的源码构建出同样的东西。不用 Docker 的话，
`cargo run --release --bin vela-relay` 可以直接运行。
...
`P256_INDEX_DOMAIN_REGISTRY` 是最容易被漏掉的一项：缺了它，
服务端发出的挑战会被合约拒绝，所有登记都会失败。它就在 `.env.example` 里，
而且必须和已部署合约自己的 `DOMAIN_REGISTRY` 一致——
从注册表 VERSION 12 起，挑战所用的域在部署时就固定下来，合约重新部署也不会变。
...
TLS 代理。`docker build -f p256-index-server/Dockerfile .` 从全新克隆的仓库也能构建成功；
如果用 Compose，先把 `.env.example` 复制成 `p256-index-server/.env`。
...
这项检查已经包含了多把钥匙的钱包所需的那两个合约，并且会单独标明它们——
一条链上没有它们，单钥匙钱包照样能跑。
