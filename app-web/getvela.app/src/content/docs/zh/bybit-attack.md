...
有两处边界要说清楚。dApp 根本无法要求 Vela 执行 `delegatecall`——网页能发起的请求只会
产生普通调用——所以 Bybit 那份载荷本身没法从这条路进来。但网页*可以*请求对你自己账户的
普通调用，比如添加一位所有者或更换模块。Vela 会像对待其他调用一样解码并展示它，目前还不会
拦截。而如果 Vela 自己的代码被替换，就像 `Safe{Wallet}` 那样，解码也就成了攻击者的解码——
这正是下一点要解决的。
