...
Vela 的 App 从不构造更换所有者的操作，所以 Vela 自己不会触发这个问题。但它仍然重要：dApp 可以请求
你的钱包更换自己的所有者（见下文“缺口”），而任何人通过其他 Safe 工具删除一把被盗的钥匙时，也不能
指望它在同一批次里就被切断。
...
- **钱包对自己的调用不会被拦截。**dApp 可以请求在你的 Safe 上执行 `enableModule`、
  `addOwnerWithThreshold`、`setFallbackHandler` 或 `setGuard`；其中任何一个，只要签一次，就会交出
  账户。Vela 会解码这些调用，但不会阻止。请拒绝任何目标是你自己地址的请求。
- **授权闸门只拦“无限”金额**（2^200 及以上；Permit2 为 2^152）。金额很大但有限的授权、签名式授权，
  或 NFT 的 `setApprovalForAll`，只会显示警示，不会被拦下。
- **获取来的描述文件没有经过认证。**来自链数据服务器的描述文件只要与合约匹配就会显示为“已验证”；
  它的可信程度取决于那台服务器。
- **独立签名页还没有接入**任何 App。
- **网络检查不会查 Safe 的通行密钥签名器工厂**，而第二到第七把钥匙需要它；在缺少它的网络上，只有第一
  把钥匙能签名。
