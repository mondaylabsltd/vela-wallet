...
这些不是合约层面的发现，而是钱包对你的保护不如你可能以为的那么多的地方。除注明是有意取舍的之外，
每一项都已记录待修：

- **无限授权只要你保留，就会发出**——这是有意的取舍，因为设了上限的授权会让 Permit2 和批量兑换无法正常工作。
  “无限”授权（2^200 及以上；Permit2 为 2^152）会以红色显示，除非你设上限，否则按 dApp 请求的原样发出。
  网页版和桌面版暂时还不能给批量交易里的授权设上限；签名式授权则在任何地方都无法设上限。
- **独立签名页还没有接入**任何 App。
