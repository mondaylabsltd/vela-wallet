...
パスキーの署名はチェーンの **RIP-7212** プリコンパイルで検証され、代わりの検証器はありません。アプリは、ネットワークを有効にする前に、実際の署名でプリコンパイルを確認します。注意点が 2 つあります。当初の RIP-7212 の仕様には境界ケースの欠陥があり、[EIP-7951](https://eips.ethereum.org/EIPS/eip-7951) で修正されています（影響するのはもともと失敗すべき入力だけで、正しい形式の WebAuthn 署名には影響しません）。また、一度の確認では、チェーンの実装がずれうるすべてのケースを見つけることはできません。
