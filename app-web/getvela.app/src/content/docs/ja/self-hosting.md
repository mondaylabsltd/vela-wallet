...
インデックスは [p256-index](https://github.com/mondaylabsltd/p256-index)（Rust）です。ウォレットが作成されると、インデックスは各鍵の証明を確認し、鍵の組を Gnosis の**レジストリコントラクト**に書き込んで、ガス代を払います。既存のレジストリ `0x94fD1A891EB6c5F340622Baf2F3A0cb70A941EA9` をそのまま使ってください。所有者がおらず、資金のあるアドレスなら誰でも書き込めて、すべての Vela のアプリがそれを直接読み取ります。自分で別のレジストリを作っても、アプリからは見えません。
...
サーバーは暗号化なしの HTTP（既定のポートは 11256）で待ち受けます。ウォレットは `https://` のエンドポイントしか受け付けないので、前段に TLS プロキシを置いてください。この文書の執筆時点では、ソースの Dockerfile はビルドできないことがあります。Cargo でのビルドは問題ありません。このリポジトリには、まだライセンスファイルがありません。
