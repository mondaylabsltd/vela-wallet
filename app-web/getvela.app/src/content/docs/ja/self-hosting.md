...
4 つのアプリはいずれも 4 つの項目すべてに従います。しかも、変更したエンドポイントは次回の起動ではなく次の呼び出しから効きます。ウォレットの作成、サインイン、アドレスの名前の検索 —— どの経路も、実際に使うその時点でエンドポイントを読むからです。iOS では、既定のインデックスに接続できないときに、サインイン画面でパスキーインデックスを変更することもできます。

（2026 年 9 月までは、これに 4 つの例外がありました。いちばんひどかったのは、仮の値を表示して何も保存しなかった iOS の画面です。いずれも直っています。）
...
どちらでも動きます。公開されているイメージがいちばん手軽で、`docker compose up --build` は、あなたが読めるソースから同じものをビルドします。Docker を使わない場合は、`cargo run --release --bin vela-relay` で直接動かせます。
...
- リレーは、各チェーンの情報と、受け付けるステーブルコインの一覧を、チェーンディレクトリから読みます。`VELA_RELAY_CHAIN_DIRECTORY_URL` に[自分のもの](#chain-data)を設定しないかぎり、読み先は `ethereum-data.getvela.app` です。この設定は vela-relay v0.9.6 以降で使えます。それより古いビルドは、常に Vela のコピーを読みます。
...
`P256_INDEX_DOMAIN_REGISTRY` は、よく見落とされる項目です。これがないと、サーバーはコントラクトが拒否するチャレンジを発行し、すべての登録が失敗します。`.env.example` に入っており、デプロイ済みコントラクト自身の `DOMAIN_REGISTRY` と一致していなければなりません。レジストリの VERSION 12 以降、チャレンジのドメインはデプロイ時に固定されるので、コントラクトを再デプロイしても変わりません。
...
サーバーは暗号化なしの HTTP（既定のポートは 11256）で待ち受けます。ウォレットは `https://` のエンドポイントしか受け付けないので、前段に TLS プロキシを置いてください。`docker build -f p256-index-server/Dockerfile .` は、クローンしたばかりのリポジトリでも通ります。Compose を使う場合は、先に `.env.example` を `p256-index-server/.env` にコピーしてください。
...
Vela は、P-256 プリコンパイルと、Vela が確認する標準コントラクトがそろった EVM チェーンなら、どこでも動きます。[チェーンのセットアップ](/ja/chain-setup)で、そのチェーンに何が足りないかがわかり、誰でもデプロイできるものはそこからデプロイできます。要件は[ネットワークと手数料](/ja/docs/networks-and-fees)で説明しています。確認には、鍵を 2 本以上持つウォレットに必要な 2 つのコントラクトも含まれており、そうであることが明示されます。それらがないチェーンでも、鍵が 1 本のウォレットは動きます。
