...
Vela는 시드 구문이 없는, **셀프 호스팅할 수 있는 이더리움 및 EVM 네트워크용 오픈소스
지갑**입니다. 지갑은 수정하지 않은 [Safe](/ko/docs/account-contract) 스마트 계정이고, 서명은
**패스키**로 합니다. 휴대폰이나 컴퓨터에 있는 패스키, 다른 휴대폰, 하드웨어 보안 키 모두 쓸
수 있습니다.

- **우리 없이도 돌아갑니다.** 앱과 그 뒤의 모든 서비스(릴레이, 공개 키 인덱스, 체인 데이터,
  환율)는 MIT 라이선스입니다. [셀프 호스팅 가이드](/ko/docs/self-hosting)를 따라 직접 빌드하고
  운영할 수 있으며, 한계도 그 가이드에 적어 두었습니다.
...
| 릴레이를 직접 운영하는 방법, 수수료는 누가 받는지 | [셀프 호스팅 가이드 → 릴레이](/ko/docs/self-hosting#relay) · [네트워크와 수수료 → 수수료](/ko/docs/networks-and-fees#fee) |
