...
네 가지 앱 모두 네 가지 항목을 전부 따릅니다. 게다가 바꾼 엔드포인트는 다음 실행이 아니라
다음 호출부터 적용됩니다. 지갑 생성, 로그인, 주소의 이름 조회 — 모든 경로가 엔드포인트를 실제로
쓰는 그 순간에 읽기 때문입니다. iOS에서는 기본 인덱스에 연결할 수 없을 때 로그인 화면에서
패스키 인덱스를 바꿀 수도 있습니다.

(2026년 9월까지는 여기에 예외가 네 가지 있었습니다. 그중 가장 심한 것은 자리 표시용
값을 보여 주고 아무것도 저장하지 않던 iOS 페이지였습니다. 모두 고쳤습니다.)
...
둘 다 됩니다. 게시된 이미지가 가장 빠르고, `docker compose up --build`는 여러분이 읽을 수 있는
소스에서 같은 것을 빌드합니다. Docker 없이 `cargo run --release --bin vela-relay`로 바로
실행할 수도 있습니다.
...
`P256_INDEX_DOMAIN_REGISTRY`는 사람들이 가장 많이 빠뜨리는 값입니다. 이 값이 없으면
서버가 컨트랙트에서 거부되는 챌린지를 내주게 되어, 모든 등록이 실패합니다. 이 값은
`.env.example`에 들어 있으며, 배포된 컨트랙트 자체의 `DOMAIN_REGISTRY`와 일치해야 합니다.
레지스트리 VERSION 12부터 챌린지 도메인은 배포 시점에 고정되므로, 컨트랙트를 다시 배포해도
바뀌지 않습니다.
...
TLS 프록시를 두세요. `docker build -f p256-index-server/Dockerfile .`는 갓 클론한
저장소에서도 잘 됩니다. Compose를 쓴다면 먼저 `.env.example`을 `p256-index-server/.env`로
복사하세요.
...
설명합니다. 이 확인에는 키가 두 개 이상인 지갑에 필요한 컨트랙트 두 개도 포함되어 있고,
그렇다는 표시도 함께 나옵니다. 그 둘이 없는 체인에서도 키가 하나인 지갑은 돌아갑니다.
