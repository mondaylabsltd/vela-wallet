...
Vela 앱은 소유자 변경을 만들지 않고, dApp이 그것을 요청해도 그 자리에서 거부되므로 Vela가 이
문제를 일으키는 일은 없습니다. 그래도 다른 Safe 도구로 유출된 키를 제거하는 사람에게는
여전히 중요합니다. 같은 번들 안에서 그 키가 차단된다고 기대할 수 없기 때문입니다.
...
컨트랙트 발견 사항은 아니지만, 지갑이 생각보다 덜 보호해 주는 부분입니다. 의도된 트레이드오프라고
밝힌 것을 빼면 모두 수정 대상으로 추적하고 있습니다.

- **무제한 승인은 그대로 두면 전송됩니다.** 의도된 트레이드오프입니다. 한도를 건 승인은 Permit2와
  배치 스왑을 깨뜨리기 때문입니다. "무제한" 승인(2^200 이상, Permit2는 2^152 이상)은 빨간색으로
  표시되고, 한도를 설정하지 않으면 dApp이 요청한 그대로 전송됩니다. 웹과 데스크톱에서는 배치 안의
  승인에 아직 한도를 설정할 수 없고, 서명 방식의 승인은 어디에서도 한도를 설정할 수 없습니다.
