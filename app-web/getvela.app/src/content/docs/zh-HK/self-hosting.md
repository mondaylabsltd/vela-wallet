...
四個 App 都完整遵從這四個欄位，而且改過的端點在下一次呼叫時就生效，不必等下一次啟動：
建立錢包、登入、按地址查詢名稱——每一條路徑都在真正用到端點的那一刻才去讀它。
在 iOS 上，預設索引無法連接時，還可以在登入畫面更改通行密鑰索引。

（在 2026 年 9 月之前，這件事有四項例外，
其中最嚴重的是 iOS 上那個顯示預留值、又甚麼都不儲存的頁面。它們都已經修好了。）
...
兩條路都走得通：已發佈的映像最省事，而 `docker compose up --build`
會從你可以讀到的原始碼建置出同樣的東西。不用 Docker 的話，
`cargo run --release --bin vela-relay` 可以直接運行。
...
- 中繼會從鏈目錄讀取每條鏈的資料和它接受的穩定幣：預設是 `ethereum-data.getvela.app`，
  ...
  這個設定需要 vela-relay v0.9.6 或以上；較舊的版本一律讀取 Vela 的那一份。
...
`P256_INDEX_DOMAIN_REGISTRY` 是最容易被漏掉的一項：
缺少它，伺服器發出的挑戰會被合約拒絕，所有登記都會失敗。它就在 `.env.example` 裡，
而且必須與已部署合約本身的 `DOMAIN_REGISTRY` 相符——
由註冊表 VERSION 12 起，挑戰所用的域在部署時就已固定，合約重新部署也不會改變。
...
`docker build -f p256-index-server/Dockerfile .` 由全新複製的儲存庫也能建置成功；
如果使用 Compose，請先把 `.env.example` 複製成 `p256-index-server/.env`。
...
要求。這項檢查已經包含持有多於一把鑰匙的錢包所需的那兩份合約，並會特別標明它們——
一條鏈上沒有它們，只有一把鑰匙的錢包照樣可以運行。
