...
- **每筆交易：**從你的錢包支付給提交交易的中繼的一筆手續費——除非你把錢包指向另一個中繼或自行運行一個，
  否則就是 Vela 的中繼。它涵蓋 gas 和中繼的利潤，最低約 0.01 美元。確切金額會在你簽署前顯示在確認畫面上，
  並且是你所簽署內容的一部分。沒有按金，也沒有訂閱費。[手續費如何計算](/zh-HK/docs/networks-and-fees#fee)。
...
是的，全部開源，一律採用 MIT 授權：錢包 App 和核心、中繼、公鑰索引、匯率服務和鏈數據目錄，
程式碼都發佈在 [GitHub](https://github.com/orgs/mondaylabsltd/repositories) 上。每一項服務你都可以
...
getvela.app 的情況下仍然可以繼續使用，而每一項服務都是開源的，其他人也可以運行。
[自行架設指南](/zh-HK/docs/self-hosting#if-getvela-app-disappears)列出了各條途徑及其限制。
