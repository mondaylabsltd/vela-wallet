...
### 自定义 scheme 回传 —— 设计已定，尚未实现（2026-09-23）

创始人提出：签名结果经一个自定义 scheme 交回 Android / iOS / 桌面。

**它不是新机制。** 核心里的 `url_launch(base, request, callback, token)` 与
`parse_callback(query, token)` 就是这个形状，只是把 `callback` 从
`http://127.0.0.1:<port>/…` 换成 `vela://sign-result?…`；一次性 token 的校验原样
适用。

**做，但作为回退，不取代回环。** 回环 WebSocket 有一条自定义 scheme 永远给不了
的性质：**独占**。端口由本进程绑定，别的 app 抢不走。自定义 scheme 谁都能注册
...
因此它适合回环不可用的场合（iOS 后台限制、桌面防火墙弹窗），**但绝不能成为唯一
的授权依据**。
...
| | 承接点 | 规则 |
| --- | --- | --- |
| Android | `singleTask` + `onNewIntent` | 不重建 Activity，不动导航栈 |
| iOS | `onOpenURL` | 只把数据交给待答复的请求，不切视图 |
| 桌面 | 系统 URL 事件 | 同上 |

一句话的规则：**回调带请求 id 与一次性 token，它只解决那一个待答复的请求；没有
待答复的就静默丢弃** —— 不弹错、不跳页。
...
> `mtu - 3 - 6` 重新定尺寸（`vela_core::clear_signer::ble::Framer::fit_to_mtu`，
> 三端共用），而不是沿用默认值。
