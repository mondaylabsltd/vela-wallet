// The mock relay against contracts/relay.md §1 — the rules the page's relay
// tests lean on. (The Rust relay has its own conformance script; this only
// makes sure the page is tested against the same room rules.)
//
//   node samples/mock-relay-test.mjs
...
const relay = await startRelay({ limits: { idleMs: 600, roomLifetimeMs: 60000, maxFrame: 1024 } });
...
try {
  ...
  const second = client(`${relay.url}/v1/rooms/${room}?role=signer`);
  await second.opened;
  ...
  const back = client(`${relay.url}/v1/rooms/${room}?role=requester`);
  await back.opened;
  ...
  const quiet = client(`${relay.url}/v1/rooms/BBBBBBBBBBBBBBBBBBBBBB?role=signer`);
  await quiet.opened;
  check('an end silent past the idle limit is closed with 4408', await until(() => quiet.closed && quiet.closed.code === 4408, 2000));
  await sleep(100);
  check('an empty room is forgotten', !relay.rooms.has('BBBBBBBBBBBBBBBBBBBBBB'));
} finally {
  await relay.close();
