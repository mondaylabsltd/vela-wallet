...
export async function tunnelWallet({ ns, tunnelUrl, room, secret, rk }) {
  ...
  const fingerprint = await secure.fingerprint(probe.publicKey);
  const wallet = {
    room,
    rk: rk || fingerprint,
    fingerprint,
  ...
  wallet.connect = () => new Promise((resolve) => {
    ...
    let handshake = null;
    let codeReady;
    wallet.codeReady = new Promise((r) => { codeReady = r; });
    ws.addEventListener('open', () => resolve(ws));
    ws.addEventListener('close', (e) => { if (wallet.ws === ws) wallet.closed = e.code; });
    ws.addEventListener('message', async (event) => {
      if (typeof event.data === 'string') {
        ...
        if (frame.t === 'hello') {
          ...
          wallet.outgoing = 0;
          wallet.lastSeen = 0;
          codeReady(wallet.code);
        }
        return;
      }
      if (!wallet.session) return;
      const plain = await wallet.session.open(new Uint8Array(event.data)).catch(() => null);
      if (!plain) return;
      const message = JSON.parse(new TextDecoder().decode(plain));
      if (typeof message.n === 'number') wallet.lastSeen = Math.max(wallet.lastSeen, message.n);
      wallet.received.push(message);
      const waiter = wallet.waiters.find((w) => w.match(message));
      if (waiter) { wallet.waiters.splice(wallet.waiters.indexOf(waiter), 1); waiter.resolve(message); }
    });
  });

  wallet.send = async (object) => {
    object.v = 1;
    object.n = Math.max(wallet.outgoing, wallet.lastSeen) + 1;
    wallet.outgoing = object.n;
    const sealed = await wallet.session.seal(new TextEncoder().encode(JSON.stringify(object)));
    wallet.ws.send(sealed);
  };
  wallet.expect = (match, timeoutMs = 30000) => new Promise((resolve, reject) => {
    const already = wallet.received.find((m) => match(m) && !m.__taken);
    if (already) { already.__taken = true; resolve(already); return; }
    const timer = setTimeout(() => reject(new Error('the page did not answer in time')), timeoutMs);
    wallet.waiters.push({ match, resolve: (m) => { clearTimeout(timer); m.__taken = true; resolve(m); } });
  });
  wallet.request = async (id, intent, context = {}) => {
    await wallet.send({ t: 'intent', id, intent, context });
    return wallet.expect((m) => m.id === id && (m.t === 'result' || m.t === 'error'));
  };
  wallet.leave = () => { try { wallet.ws.close(1000); } catch { /* gone */ } };
  return wallet;
}
