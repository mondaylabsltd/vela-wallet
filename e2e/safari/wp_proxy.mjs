...
// relay closes the peer — the last-hop difference versus a clean Node protocol peer.
//
...
const roleOf = (path) => {
  const name = new URL(path, 'ws://x').searchParams.get('name') || '';
  return name === 'Vela Concurrency Peer' ? 'PEER' : name === 'Vela Wallet' ? 'APP ' : '????';
};
