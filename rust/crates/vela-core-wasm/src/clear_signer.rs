//! What the web wallet still needs of the Clear Signer (spec 075): nothing of
//! the channel, only the REGISTRY's relying party.
//!
//! The web wallet does not offer the Clear Signer at all (owner, 2026-09-23 —
//! "web 就不支持清晰签名器好了"): no page, no ceremonies, no loopback socket.
//! But a wallet whose keys were minted on a phone's Clear Signer page can be
...
//! 「清晰签名器的回复与这笔请求不符」, so the rule stays where every shell can
//! reach it.
...
#[wasm_bindgen(js_name = clearSignerRegistryRpId)]
...
#[wasm_bindgen(js_name = clearSignerUnitRpId)]
pub fn clear_signer_unit_rp_id(
    ...
    clear_signer::registry_unit_rp_id(&origins, wallet_rp_id)
