//! Spec 075: the Clear Signer's ceremonies besides signing — what the core
...
use vela_core::clear_signer::ceremony::{request, verify, Answer, Ceremony};
use vela_core::clear_signer::ClearSignerError;
...
fn a_sign_in_takes_only_a_challenge_the_page_derived() {
    ...
    assert_eq!(
        ...
        Err(ClearSignerError::WrongChallenge)
    );
    assert_eq!(
        verify(
            &sign_in(),
...
fn a_proof_takes_its_purposes_prefix_and_its_own_key() {
    ...
    assert_eq!(
        ...
        Err(ClearSignerError::ForeignKey)
    );
}
...
fn an_unverified_user_or_a_foreign_origin_is_refused() {
    assert_eq!(
        ...
        Err(ClearSignerError::NotVerified)
    );
    assert!(matches!(
        verify(
            &sign_in(),
...
fn a_declined_or_refused_page_says_so() {
    ...
    assert_eq!(
        ...
        Err(ClearSignerError::Refused("refused".to_owned()))
    );
}
