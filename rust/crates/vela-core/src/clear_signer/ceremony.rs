//! Spec 075: the Clear Signer as a passkey route — the ceremonies besides
...
//! With `method = clear_signer` the shell hands the operation here, sends the
...
/// The four passkey operations the Clear Signer runs, with only the fields a
...
fn field<'a>(object: &'a Value, name: &str) -> Result<&'a str, ClearSignerError> {
    object
        .get(name)
...
fn hex_bytes(object: &Value, name: &str) -> Result<Vec<u8>, ClearSignerError> {
    crate::primitives::from_hex(field(object, name)?)
        .map_err(|_| ClearSignerError::Malformed(format!("{name} is not hex")))
}

fn credential_hex(object: &Value) -> Result<String, ClearSignerError> {
    URL_SAFE_NO_PAD
...
fn client_data(json: &[u8]) -> Result<Value, ClearSignerError> {
...
fn same_origin(client: &Value, signer_origin: &str) -> Result<(), ClearSignerError> {
    let seen = client
        .get("origin")
        .and_then(Value::as_str)
        .unwrap_or_default();
    if origin_of(seen).is_empty() || origin_of(seen) != origin_of(signer_origin) {
        return Err(ClearSignerError::Malformed(format!(
            "the answer came from {seen}, not from the Clear Signer page"
        )));
    }
    Ok(())
}

fn challenge_bytes(client: &Value) -> Result<Vec<u8>, ClearSignerError> {
    let text = client
        .get("challenge")
        .and_then(Value::as_str)
        .unwrap_or_default();
    URL_SAFE_NO_PAD
        ...
        .map_err(|_| ClearSignerError::WrongChallenge)
}
...
fn verify_registration(answer: &Value, signer_origin: &str) -> Result<Answer, ClearSignerError> {
    let registration = answer
        .get("registration")
        .ok_or_else(|| ClearSignerError::Malformed("missing registration".into()))?;
    let client_data_json = hex_bytes(registration, "clientDataJSON")?;
    validate_client_data(ClientDataKind::Create, &client_data_json, &[])
        .map_err(|error| ClearSignerError::Malformed(error.to_string()))?;
...
///
/// # Errors
/// Every [`ClearSignerError`] but `WrongToken`, which is the channel's to
...
pub fn verify(
...
) -> Result<Answer, ClearSignerError> {
    ...
    let assertion = answer
        .get("assertion")
        .ok_or_else(|| ClearSignerError::Malformed("missing assertion".into()))?;
    ...
    if let Some(named) = named {
        ...
        {
            return Err(ClearSignerError::ForeignKey);
        }
    }
