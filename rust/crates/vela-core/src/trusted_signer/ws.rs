//! The phones' channel to the Trusted Signer: a WebSocket on the wallet's own
//! loopback (spec 071).
//!
...
use super::{refusal, TrustedSignerError};
...
pub struct Step {
    ...
    pub outcome: Option<Result<Value, TrustedSignerError>>,
}
...
impl Connection {
            ...
            match frame {
                ...
                Frame::Close => {
                    if self.phase == Phase::Answer {
                        step.outcome = Some(Err(TrustedSignerError::Declined));
                    }
    ...
    pub fn closed(&mut self) -> Option<TrustedSignerError> {
        let phase = std::mem::replace(&mut self.phase, Phase::Done);
        (phase == Phase::Answer).then_some(TrustedSignerError::Declined)
    }
    ...
    fn fail(&mut self, step: &mut Step, code: u16) {
        if self.phase == Phase::Answer {
            step.outcome = Some(Err(TrustedSignerError::Malformed(format!(
