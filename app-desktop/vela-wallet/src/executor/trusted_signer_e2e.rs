...
impl Browser {
    ...
    fn launch() -> Option<Self> {
        ...
        let base = std::env::var("TRUSTED_SIGNER_E2E_DIR")
...
impl Rig {
    fn new() -> Option<Self> {
        let Some(browser) = Browser::launch() else {
            eprintln!("CHROME_BIN is not set — scripts/trusted-signer-e2e.sh sets it");
            return None;
        };
        ...
        let keys = vec![
            ...
            crate::executor::trusted_signer::tests::wallet_key(&signing_key(9), &[0x99]),
