...
impl IntroArtCache {
    ...
    pub fn image(&mut self, art: IntroArt, palette: Palette, logical_w: u32) -> Arc<RenderImage> {
        ...
        let image =
            rasterize(&svg_document(art, &palette), w, h).unwrap_or_else(empty_render_image);
...
mod tests {
    ...
    fn every_illustration_is_the_contract() {
        ...
        for art in IntroArt::ALL {
            ...
            for (i, (w, h)) in want.iter().zip(have).enumerate() {
                ...
                assert_eq!(
                    w["opacity"].as_f64().unwrap() as f32,
                    h.opacity,
                    "{}[{i}]: opacity",
                    art.id()
                );
                assert_eq!(
                    w["role"].as_str().unwrap() == "accent",
                    h.role == Role::Accent,
                    "{}[{i}]: role",
                    art.id()
                );
    ...
    fn every_illustration_rasterises() {
        ...
        for art in IntroArt::ALL {
            assert!(
                rasterize(&svg_document(art, &palette), 160, 128).is_some(),
                "{art:?}"
            );
        }
    }
