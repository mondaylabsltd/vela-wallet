 ...
 */
internal class TrustedSignerEnvelopes {
 ...
 */
internal fun judgeTrustedSignerAnswer(
    ask: TrustedSignerAsk,
    answer: JSONObject,
    signerUrl: String,
): TrustedSignerAnswer = when (ask) {
    is TrustedSignerAsk.Signature -> {
        if (answer.optString("t") == "error") {
            TrustedSignerAnswer.Signed(TrustedSignerOutcome.Refused(trustedSignerRefusalOf(answer.optString("code"))))
        } else {
            val result = answer.optJSONObject("result")
            if (result == null) {
                TrustedSignerAnswer.Signed(
                    TrustedSignerOutcome.Refused(TrustedSignerRefusal.Malformed("no result")),
                )
            } else {
                TrustedSignerAnswer.Signed(trustedSignerVerify(result.toString(), ask.digest, ask.keys))
            }
        }
    }
...
/** `trusted_signer::refusal` — the person, or the page's own rules. */
internal fun trustedSignerRefusalOf(code: String): TrustedSignerRefusal =
    if (code.isEmpty() || code == "user_rejected") {
        TrustedSignerRefusal.Declined
    } else {
        TrustedSignerRefusal.PageRefused(code)
    }
