> **History (2026-09-11).** This document described the i18n architecture of the React Native / Expo app (`i18next` + `react-i18next` + `expo-localization`, `src/i18n/`), retired and deleted in spec 039. The corpus it describes is still the single source of truth — `rust/crates/vela-core/i18n/locales/`, generated by `scripts/gen-i18n.mjs` into the Rust catalogs and `assets/i18n/` — and the living rules are in `app-web/vela-wallet/CLAUDE.md` (rule 2–3), `rust/crates/vela-core/src/i18n/`, and the 6-step corpus procedure in the memory notes. Kept as the record of the format decisions (presets, not `Intl`; the regional-format screen).
