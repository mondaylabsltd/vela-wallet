...
**2. Positioning & taglines (use verbatim).** Brand line: **"An Ethereum wallet you actually own."** *Approved per-locale copy now lives in `app-web/getvela.app/src/lib/i18n/messages/<tag>.json`, with the founder-approved lines recorded in `specs/059-getvela-site-i18n/approved-copy.md` — only `en` and `zh` are approved; the rest are drafted. `bun run i18n:status` says which.* (Simplified Chinese, verbatim: **"真正属于你的以太坊钱包"**.) *Retired 2026-09-15 by spec 059: "Your keys. Your face." — never reinstate it.* Product: **"an open-source, self-custodial wallet for ETH and ERC-20s."** Secondary: **"A wallet that does less — on purpose."** Source: `app-web/getvela.app/src/routes/+page.svelte`. **Never improvise alternative taglines.**
